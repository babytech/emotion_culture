...
M3 的前端交互以“用户主动切换静态风格图”为准，不再使用第三方动态生图，也不再依赖自拍授权链路。
...
- 目标文件：
- `docs/stage1/phase3-qa-regression-report.md`
- 完成定义：
- 四类场景回归可复现并通过：
- 拒绝授权
- 超周限额
...
## 里程碑 M3：小程序静态风格切换上线（功能层）

### MINI-301 用户主动触发媒体增强

- 目标：用户点击后才触发静态风格图切换任务。
- 目标文件：
...
- 目标：去除“自拍参考/第三方增强/授权弹窗”语义，切换功能不依赖自拍授权。
- 目标文件：
- `apps/wechat-mini/pages/result/*`
- 完成定义：
- 页面不再出现“会使用自拍参考”文案
...
- 目标文件：
- `apps/wechat-mini/pages/result/*`
- 完成定义：支持 `classical` / `guochao` / `tech` 风格参数传递
- 说明：
- `classical` 映射诗人/唐宋静态图池
- `guochao` 映射国潮静态图池
- `tech` 预留科技静态图池，未配置时前端明确提示“暂未支持”

### MINI-304 状态反馈统一

- 目标：统一展示“切换中/失败/重试/完成”。
- 目标文件：
- `apps/wechat-mini/pages/result/*`
- `apps/wechat-mini/services/api.js`
