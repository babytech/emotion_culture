...
function buildRecoverableAnalyzeError(err) {
  ...
  if (message.includes("分析任务仍在处理中")) {
    return "分析任务仍在云端处理中，可稍后再次点击“开始分析”继续查询，无需重新上传自拍或录音。";
  }
...
Page({
  ...
  withWorkspaceMetrics(partial = {}) {
    ...
    const nextText = typeof nextState.text === "string" ? nextState.text : "";
    return {
      ...partial,
      textLength: nextText.length,
      readyInputCount:
        (nextText.trim() ? 1 : 0) +
        (nextState.imageTempPath ? 1 : 0) +
        (nextState.audioTempPath ? 1 : 0),
    };
  },
  ...
  async pollAnalyzeTaskResult(taskId, options = {}) {
    ...
    throw new Error("分析任务仍在处理中，请稍后再次点击“开始分析”继续查询，或到历史记录页查看结果。");
  },
  ...
  async submitAnalyze() {
    ...
    if (pendingSelfiePath && !imageTempPath) {
      this.setData({ errorMsg: "请先确认自拍照片，再开始分析。" });
      return;
    }
